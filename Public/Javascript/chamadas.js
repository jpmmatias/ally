...
let VideoChat = {
	...
	requestMediaStream: function(event) {
		...
		navigator.mediaDevices
			...
			.catch((error) => {
				logIt(error);
				logIt('Falha ao obter vídeo da webcam local, verifique as configurações de privacidade da webcam');
...
function recieveCaptions(captions) {
	...
	if (captions === 'notusingchrome') {
		alert('Outro chamador deve estar usando o Chrome para que este recurso funcione. Live Caption desativada.');
		receivingCaptions = false;
		captionText.text('').fadeOut();
		captionButtontext.text('Start Live Caption');
		return;
	...
	if ('pictureInPictureEnabled' in document || remoteVideoVanilla.webkitSetPresentationMode) {
		if (document.pictureInPictureElement) {
			...
			remoteVideoVanilla.requestPictureInPicture().catch((error) => {
				alert('Você deve estar conectado a outra pessoa para entrar imagem na imagem.');
			});
		}
	} else {
		alert('Imagem em imagem não é compatível com seu navegador. Considere usar o Chrome ou Safari.');
	}
}
