	...
	try {
		...
		if (!teste) {
			...
			if (!tester) {
				...
				let userCriador = await User.findOneAndUpdate(
					{ _id: userCridadorId, mandarConvite: { $nin: filterCria } },
					{ $push: updateUserCridador },
					{
						new: true
					}
				).lean();
...
exports.aceitarConviteTeste = async (req, res, next) => {
	...
	try {
		let testeConvite = await Teste.findById(testeId);
		...
		let userCriador = await User.findOneAndUpdate(
			filtroUser,
			{ $pull: { mandarConvite: userCriadorMandarConvite } },
			{ new: true }
		);
